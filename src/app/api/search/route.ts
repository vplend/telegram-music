/**
 * API-роут для поиска песен через Genius API
 * GET /api/genius?query=...
 * Возвращает массив песен с названием, артистом, картинкой и url
 */
export async function GET(req: Request) {
  ...
  const songs = hits.map((hit: { result: { title: string; artist_names?: string; song_art_image_thumbnail_url?: string; url: string; release_date_components?: { year: string }} }) => {
